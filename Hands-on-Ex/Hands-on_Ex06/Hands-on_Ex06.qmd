...
Statistical conclusion: Standardization (Min-Max or Z-score) changes the scale but preserves the underlying distribution shape of `RADIO_PR`.
...
he choropleth map above reveals the clusters are very fragmented. The is one of the major limitation when non-spatial clustering algorithm such as hierarchical cluster analysis method is used.
